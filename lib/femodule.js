...
let femodule = {
  ...
  uploadModule(module_path){
    ...
    return new Promise((resolve, reject)=>{
      ...
      if(config.module_author == undefined || config.module_author == ''){
  ...
  uploadModuleByPath(module_path, module_json){
    ...
    return new Promise((resolve, reject)=>{
      ...
      request.post({url: config.module_server_url + '/modules/uptest', formData: formData}, function optionalCallback(err, httpResponse, body) {
        if (err) {
          reject(err)
        }
        else{
  ...
  downloadModule(module_name, module_version){
    let config = appdata.get()
    let module_server_url = config.module_server_url
    return new Promise((resolve, reject)=>{
